...
const Update = () => {
  ...
  useEffect(() => {
    const fetchCurrencyData = async () => {
      try {
        const response = await axios.get(`https://server-le71.onrender.com/api/currencies/name/${name}`);
  ...
  const updateCurrency = async (e) => {
    ...
    try {
      const res = await axios.put(`https://server-le71.onrender.com/api/currencies/updateByName/${name}`, updatedCurrency);
